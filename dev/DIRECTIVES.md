# iqdb-filter -- Engineering Directives
...
iqdb-filter combines metadata filtering with vector similarity efficiently. The hard part it solves is applying a filter during an HNSW or IVF traversal without destroying recall. It evaluates the filter language defined in `iqdb-types`, estimates selectivity, and chooses among pre-, post-, and in-traversal strategies automatically.
...
- `iqdb-types`: the `Filter`, `Metadata`, and `Value` types
- `iqdb-hnsw` / `iqdb-ivf`: consume this for filtered traversal
